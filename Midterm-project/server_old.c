...
void* start_client_exec(void* dump){
    ...
    while(1){
        ...
        if(in_call[my_id-1]==3){
            /*int error;
            pa_usec_t latency;
            if ((latency = pa_simple_get_latency(s_write, &error)) == (pa_usec_t) -1) {
                fprintf(stderr, __FILE__": pa_simple_get_latency() failed: %s\n", pa_strerror(error));
            }
            fprintf(stderr, "%f usec\n", (float)latency);*/
            
            if(read(other_clients[my_id-1],call_buff,sizeof(call_buff))<0){
                
            }
            if(write(other_clients[receiver_id-1],call_buff,sizeof(call_buff))<0){
                
            }
            
            if(call_buff[0]=='-' && call_buff[1]=='v' && call_buff[3]=='q'){
                receiver_id=0;
                in_call[my_id-1]=0;
            }
        ...
        if(buff[0]=='-'){
            ...
            if(buff[1]=='s'){
                ...
                if(buff[3]=='y' && in_call[my_id-1]%4==1){
                    printf("OK1\n");
                    receiver_id=in_call[my_id-1]/4;
                    in_call[my_id-1]=3;
                    in_call[receiver_id-1]=3;
                    write(other_clients[receiver_id-1],"-m Call Started.\0",strlen("-m Call Started.\0")+1);
                    write(other_clients[receiver_id-1],"-v y\0",strlen("-v y\0")+1);
                    write(other_clients[my_id-1],"-v y\0",strlen("-v y\0")+1);
                }
                else if(buff[3]=='n' && in_call[my_id-1]%4==1){
                    printf("OK2\n");
                    receiver_id=in_call[my_id-1]/4;
                    in_call[my_id-1]=0;
                    in_call[receiver_id-1]=0;
                    write(other_clients[receiver_id-1],"-m Rejected.\0",strlen("-m Rejected.\0")+1);
                    write(other_clients[receiver_id-1],"-v n\0",strlen("-v n\0")+1);
                    receiver_id=0;
                }
                else if(buff[3]=='a' && in_call[my_id-1]%4==1){
                    printf("OK3\n");
                    receiver_id=in_call[my_id-1]/4;
                    in_call[my_id-1]=0;
                    in_call[receiver_id-1]=0;
                    write(other_clients[receiver_id-1],"-m In Different Call.\0",strlen("-m In Different Call.\0")+1);
                    write(other_clients[receiver_id-1],"-v a\0",strlen("-v a\0")+1);
                    receiver_id=0;
                }
                else{
                    printf("OK4\n");
                    if(receiver_id==0){
                        write(other_clients[my_id-1],"-m Select a receiver first.\0",strlen("-m Select a receiver first.\0")+1);
                    }
                    else{
                        in_call[receiver_id-1]=4*(my_id)+1;
                        in_call[my_id-1]=4*(receiver_id)+2;
                        write(other_clients[receiver_id-1],"-v\0",strlen("-v\0")+1);
                    }
                }
            }
